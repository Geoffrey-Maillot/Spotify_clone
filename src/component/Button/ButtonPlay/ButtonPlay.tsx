...
const ButtonPlay = ({ size = 'small' }: Props) => {
  const sizes: { [x: string]: string } = {
    small: 'w-[3.125rem] h-[3.125rem] hover:scale-110',
    large: 'w-[3.5rem] h-[3.5rem] hover:scale-110',
  };
...
// Return =>
  return (
    ...
    >
      <GrPlayFill className='translate-x-0.5' size={size === 'large' ? '1.3rem' : ''} color="#000" />
    </button>
  );
};
